            ...
            {slide?.content ? (
              ...
              <div className="w-full h-full flex flex-col items-center justify-center p-12 text-slate-800">
                ...
                <div className="w-full border-b-2 border-slate-800 pb-4 mb-6">
                  <h1 className="text-3xl font-bold text-slate-900">{slide?.title || "Slide Title"}</h1>
                </div>
                <div className="w-full flex-1 space-y-4">
                  <div className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                    <p className="font-semibold text-slate-700 mb-2">Generated Content:</p>
                    <p className="text-lg text-slate-900">{slide?.title}</p>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                      <p className="text-sm font-medium text-slate-500 mb-1">Layout</p>
                      <p className="text-slate-900">{slide?.layout || "Standard"}</p>
                    </div>
                    <div className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                      <p className="text-sm font-medium text-slate-500 mb-1">Generated</p>
                      <p className="text-slate-900">{slide?.generatedAt ? new Date(slide.generatedAt).toLocaleDateString() : "Now"}</p>
                    </div>
                  </div>
                </div>
                <div className="w-full mt-6 pt-4 border-t border-slate-100">
                  <p className="text-xs text-slate-400 text-center">
                    Generated by SlideTheory
                  </p>
                </div>
              </div>
            )}
